...
#define pixel_src_to_rgba pixel_rgb888_to_rgba
...
ret_t blend_image_bgra8888_rgb888(bitmap_t* dst, bitmap_t* src, const rectf_t* dst_r, const rectf_t* src_r, uint8_t a) 
{
    return_value_if_fail(dst != NULL && src != NULL && src_r != NULL && dst_r != NULL, RET_BAD_PARAMS);
    return_value_if_fail(dst->format == BITMAP_FMT_BGRA8888 && src->format == BITMAP_FMT_RGB888, RET_BAD_PARAMS);
...
#if 0
ret_t blend_image_rotate_bgra8888_rgb888(bitmap_t* dst, bitmap_t* src, const rectf_t* dst_r, const rectf_t* src_r, uint8_t a, orientation_t o) 
{
    return_value_if_fail(dst != NULL && src != NULL && src_r != NULL && dst_r != NULL,RET_BAD_PARAMS);
    return_value_if_fail(dst->format == BITMAP_FMT_BGRA8888 && src->format == BITMAP_FMT_RGB888, RET_BAD_PARAMS);
