...
#if 0
    /* 20240318 by ql for solve SIGPIPE, SO_NOSIGPIPE 并不是一个标准的选项，因此不被Linux支持 */
    int opt = 1;

    if (setsockopt(fd, SOL_SOCKET, SO_NOSIGPIPE, (void*)&opt, sizeof(opt)) != 0)
        printf("Could not set SO_NOSIGPIPE for socket %d", fd);
#endif
