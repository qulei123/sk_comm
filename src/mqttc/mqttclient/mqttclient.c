...
#include "mqtt_log.h"           /* mqtt log 不对外开放，仅内部使用 */
...
/*  函数定义
 *  MQTT_CLIENT_SET_DEFINE(client_id, char*)
 *  char* mqtt_set_client_id(mqtt_client_t *c, char *t);
 *  {
 *      c->mqtt_client_id = t; 
 *      return c->mqtt_client_id;
 *  }
 */
...
static int mqtt_send_packet(mqtt_client_t* c, int length, platform_timer_t* timer)
{
    ...
    while ((sent < length) && (!platform_timer_is_expired(timer))) {
        ...
        {
            /* 20240318 by ql for solve SIGPIPE */
            if (errno == EPIPE)
            {
                MQTT_LOG_W("Broken pipe, disconnect ... \n");
                /* 通过心跳判断网络是否断开,此处不处理 */
...
/* 20240312 by ql for resend */
int mqtt_send_packet_with_data(mqtt_client_t *c, uint8_t *data, int length)
{
    ...
    while ((sent < length) && (!platform_timer_is_expired(&timer)))
    {
        len = network_write(c->mqtt_network, data, length, platform_timer_remain(&timer));
        if (len <= 0)  // there was an error writing the data
        {
            /* 20240318 by ql for solve SIGPIPE */
            if (errno == EPIPE)
    ...
    if (rc == MQTT_SUCCESS_ERROR) {
        if(NULL == c->mqtt_thread) {
            ...
            if (NULL != c->mqtt_thread) {
                ...
                if (c->mqtt_connect_callback)
                {
                    c->mqtt_connect_callback(c);
                }
...
static int mqtt_init(mqtt_client_t* c)
{
    ...
    c->mqtt_reconnect_try_duration = MQTT_RECONNECT_DEFAULT_DURATION;   /* 重连时间间隔 */

    c->mqtt_will_options = NULL;
    c->mqtt_reconnect_data = NULL;
    c->mqtt_reconnect_handler = NULL;
    c->mqtt_interceptor_handler = NULL;
    c->mqtt_connect_callback = NULL;
    c->mqtt_disconnect_callback = NULL;
    ...
    /* 调用者进行消息重发时需要使用write_buf，否则需要重新序列化，所以此处使用递归互斥锁 */
    platform_mutex_init(&c->mqtt_write_lock, PTHREAD_MUTEX_RECURSIVE);  /* PTHREAD_MUTEX_RECURSIVE */
    platform_mutex_init(&c->mqtt_global_lock, PTHREAD_MUTEX_NORMAL);    /* PTHREAD_MUTEX_ERRORCHECK */
...
MQTT_CLIENT_SET_DEFINE(connect_callback, connect_callback_t, NULL)
MQTT_CLIENT_SET_DEFINE(disconnect_callback, disconnect_callback_t, NULL)
...
/* mqtt broker不回复PINGRESP，mqtt_ping_outstanding计数不会请0，认为是断网 */
int mqtt_keep_alive(mqtt_client_t* c)
{
    ...
    if (platform_timer_is_expired(&c->mqtt_last_sent) || platform_timer_is_expired(&c->mqtt_last_received)) {
        if (c->mqtt_ping_outstanding) {
            ...
            mqtt_set_client_state(c, CLIENT_STATE_DISCONNECTED);
            if (c->mqtt_disconnect_callback)
            {
                c->mqtt_disconnect_callback(c);
            }
...
int mqtt_publish(mqtt_client_t* c, const char* topic_filter, mqtt_message_t* msg)
{
...
#if 0
    /* payloadlen长度小于mqtt序列化后的长度，mqtt_write_buf_size是mqtt序列化buffer的大小，所以这里判断意义不大 */
    if (msg->payloadlen > c->mqtt_write_buf_size) {
        MQTT_LOG_E("publish payload len[%d] is is greater than client write buffer[%d]...", msg->payloadlen, c->mqtt_write_buf_size);
        RETURN_ERROR(MQTT_BUFFER_TOO_SHORT_ERROR);
    }
#endif
    ...
    if (len <= 0)
    {
        if (len == MQTTPACKET_BUFFER_TOO_SHORT)
            rc = MQTT_BUFFER_TOO_SHORT_ERROR;
        goto exit;
    }
...
exit:
    ...
    /* 20240314 by ql for get send data serialize len */
    if (MQTT_SUCCESS_ERROR == rc)
    {
        rc = len;
    }
