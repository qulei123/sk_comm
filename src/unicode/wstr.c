...
wchar_t* wcs_dup(const wchar_t* s) {
  wchar_t* s1 = calloc(sizeof(wchar_t), wcslen(s) + 1);
...
ret_t wstr_extend(wstr_t* str, uint32_t capacity) {
  ...
  if (capacity > 0) {
    wchar_t* s = realloc(str->str, (capacity + 1) * sizeof(wchar_t));
...
ret_t wstr_set_utf8_with_len(wstr_t* str, const char* text, uint32_t len) {
  ...
  utf8_to_utf16_ex(text, len, str->str, str->capacity - 1);
...
ret_t wstr_set_utf8_no_alloc(wstr_t* str, const char* text)
{
    return_value_if_fail(str != NULL && text != NULL, RET_BAD_PARAMS);

    utf8_to_utf16(text, str->str, str->capacity);
    str->size = wcslen(str->str);
...
ret_t wstr_push_int(wstr_t* str, const char* format, int32_t value) {
  char buff[NUM_MAX_LEN + 1];
  wchar_t s[NUM_MAX_LEN + 1];

  memset(s, 0x00, sizeof(s));
  memset(buff, 0x00, sizeof(buff));
  return_value_if_fail(str != NULL && format != NULL, RET_BAD_PARAMS);

  snprintf(buff, NUM_MAX_LEN, format, value);
  utf8_to_utf16(buff, s, NUM_MAX_LEN);
...
ret_t wstr_to_int(wstr_t* str, int32_t* v) {
  return_value_if_fail(str != NULL && v != NULL, RET_BAD_PARAMS);
  if (str->size > 0) {
    char buff[NUM_MAX_LEN + 1] = {0};
    wchar_t wbuff[NUM_MAX_LEN] = {0};
    uint32_t size = min(str->size, NUM_MAX_LEN) * sizeof(wchar_t);
    memcpy(wbuff, str->str, size);
    utf8_from_utf16_ex(wbuff, ARRAY_SIZE(wbuff), buff, ARRAY_SIZE(buff));
    *v = tk_atoi(buff);
  } else {
    *v = 0;
  }
...
ret_t wstr_to_float(wstr_t* str, double* v) {
  ...
  if (str->size > 0) {
    ...
    memcpy(wbuff, str->str, size);
    if (utf8_from_utf16_ex(wbuff, ARRAY_SIZE(wbuff), buff, ARRAY_SIZE(buff)) != NULL) {
      *v = tk_atof(buff);
    } else {
      *v = 0.0f;
      return RET_FAIL;
...
ret_t wstr_add_float(wstr_t* str, double delta) {
  ...
  uint32_t fraction_nr = max(fraction_nr1, fraction_nr2);
  return_value_if_fail(str != NULL && str->str != NULL, RET_BAD_PARAMS);
