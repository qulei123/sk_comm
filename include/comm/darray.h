...
typedef ret_t (*visit_t)(void* ctx, const void* data);
...
typedef struct _darray_t 
{
    ...
    uint32_t capacity;        /* 数组的容量大小 */
    void** elms;              /* 数组中的元素 */
    destroy_t destroy;        /* 元素销毁函数 */
    compare_t compare;        /* 元素比较函数 */
} darray_t;
 ...
 * @return {darray_t*} 数组对象。
 */
darray_t* darray_init(darray_t* darray, uint32_t capacity, destroy_t destroy,
                      compare_t compare);
 ...
 * @return {int32_t} 如果找到，返回满足条件的对象的位置，否则返回-1。
 */
int32_t darray_find_index_ex(darray_t* darray, compare_t cmp, void* ctx);
