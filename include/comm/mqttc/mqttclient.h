...
typedef int (*connect_callback_t)(void *client);
typedef int (*disconnect_callback_t)(void *client);
...
typedef struct mqtt_client {
    ...
    uint32_t                    mqtt_cmd_timeout;               /* 一条mqtt命令/一包数据的超时时间 */
    ...
    connect_callback_t          mqtt_connect_callback;
    disconnect_callback_t       mqtt_disconnect_callback;
} mqtt_client_t;
...
MQTT_CLIENT_SET_STATEMENT(connect_callback, connect_callback_t)
MQTT_CLIENT_SET_STATEMENT(disconnect_callback, disconnect_callback_t)
